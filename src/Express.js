...
app.post('/login', (req, res) => {
  ...
  User.findOne({ username }, (error, user) => {
    if (error) {
      ...
      return res.status(500).send('<h2>Error finding user</h2>');
    }
    if (!user) {
      // User not found
      return res.status(404).send('<h2>User not found. Please register first.</h2>');
    }
    ...
    bcrypt.compare(password, user.password, (error, isMatch) => {
      if (error) {
        ...
        return res.status(500).send('<h2>Error logging in</h2>');
      }
...
app.post('/register', (req, res) => {
  ...
  bcrypt.genSalt(10, (error, salt) => {
    if (error) {
      ...
      return res.status(500).send('<h2>Error registering user</h2>');
    }
    ...
    bcrypt.hash(password, salt, (error, hashedPassword) => {
      ...
      newUser.save((error) => {
        ...
        res.status(200).send('<h2>Registration successful! <br> Click here to login now <a href="/">Login</a></h2>');
      });
    });
  });
});
